...
final class WatchSessionManager: NSObject, ObservableObject, WCSessionDelegate {
    ...
    private override init() {
        ...
        print("⌚ WCSession activate çağrılıyor")
        session.activate()
    }
    ...
    func session(
        ...
        DispatchQueue.main.async {
            ...
            if let error = error {
                ...
                print(
                    "❌ WCSession activation hata: \(error.localizedDescription)"
                )
            ...
            print(
                "✅ WCSession activation state: \(activationState.rawValue)"
            )
        }
    }
    ...
    func sessionReachabilityDidChange(_ session: WCSession) {
        DispatchQueue.main.async {
            ...
            print("📱 Reachability değişti: \(session.isReachable)")
        }
    }
            ...
            replyHandler: { reply in
                DispatchQueue.main.async {
                    ...
                    if success {
                        switch command {
                        case "IGNITION_ON":
                            self.lastResult = "Kontak açıldı ✅"
                        case "IGNITION_OFF":
                            self.lastResult = "Kontak kapandı ✅"
                        case "START":
                            self.lastResult = "Marş basıldı ✅"
                        case "SEAT_OPEN":
    ...
    private func applyStatus(_ data: [String: Any]) {
        ...
        DispatchQueue.main.async {
            ...
            if let ignition =
                data["ignitionOn"] as? Bool {

                self.ignitionOn = ignition
            }

            if let starter =
                data["starterActive"] as? Bool {

                self.starterActive = starter
            }

            if let seat =
                data["seatActive"] as? Bool {

                self.seatActive = seat
            }

            if let voltage =
                data["batteryVoltage"] as? Double {
            ...
            print(
                "⌚ UI STATUS → " +
                "ignition=\(self.ignitionOn), " +
                "starter=\(self.starterActive), " +
                "seat=\(self.seatActive), " +
                "battery=\(self.batteryVoltage)"
            )
        }
    }
