...
final class WatchSessionManager: NSObject, ObservableObject, WCSessionDelegate {
    ...
    func session(
        ...
        guard let command = message["command"] as? String else {
            replyHandler(["success": false, "error": "Komut bulunamadı"])
            return
        }
    ...
    func sendStatusToWatch(
        ...
        do {
            ...
            print("❌ Watch context hatası: \(error.localizedDescription)")
        }
        ...
        if session.isReachable {
            session.sendMessage(
                ...
                errorHandler: { error in
                    print("❌ Watch canlı status hatası: \(error.localizedDescription)")
                }
            )
        }
    }
